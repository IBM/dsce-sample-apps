...
# Narrative Quality metric
prompt_template_narrative = """You are an expert evaluator. Your job is to assess whether a narrative accurately summarizes a JSON table given a question.
...
Compare the question, the JSON data, and the narrative summary. You must determine whether the narrative accurately and completely summarizes the relevant information from the JSON table to answer the question.

## Grading Scale:
Rate the narrative quality using one of these four levels:

excellent - The narrative accurately extracts and summarizes all relevant data from the JSON table, directly answering the question with correct facts and figures.
good - The narrative addresses the question and uses information from the JSON table correctly, but may have minor omissions of non-critical details.
fair - The narrative partially addresses the question but has some inaccuracies or misses important information from the JSON table.
poor - The narrative contains significant inaccuracies, misses key data from the JSON table, or fails to properly answer the question.
"""

options_narrative_quality = {"excellent": 1.0, "good": 0.75, "fair": 0.5, "poor": 0.0}

narrative_quality = LLMAsJudgeMetric(
    name="narrative_quality",
    display_name="Narrative Quality",
    prompt_template=prompt_template_narrative,
    options=options_narrative_quality,
    llm_judge=llm_judge,
)
...
def get_available_metrics():
    ...
    # Add custom metrics from gov_custom_metrics.py (using ibm_watsonx_gov)
    try:
        ...
        metrics["Narrative Quality (LLM Judge)"] = {
            ...
            "description": "Evaluates if narrative accurately summarizes JSON table data given a question (LLM Judge)",
            "column_name": "narrative_quality",
            "category": "quality",
        }
...
if __name__ == "__main__":
    SERVICE_PORT = os.getenv("SERVICE_PORT", default="8070")
